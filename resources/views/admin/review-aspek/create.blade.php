...
@section('header')
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Aspek Penilaian</h2>
@endsection
    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-4 sm:p-6">
                <form action="{{ route('admin.review-aspek.store') }}" method="POST">
                    @csrf
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="nama" id="nama" value="{{ old('nama') }}"
                                   ...
                                   placeholder="Contoh: Kesesuaian Isi" required>
                            @error('nama')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <p class="text-xs text-gray-500">Urutan akan diatur otomatis di akhir daftar. Anda bisa mengubahnya dengan tombol panah di halaman daftar aspek.</p>
                    ...
                    <div class="mt-6 flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
                        ...
                        <button type="submit" class="inline-flex justify-center bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
