...
@section('header')
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        {{ __('Edit Periode Tahap Penyusunan') }}
    </h2>
@endsection

@section('content')
@php
    $firstTahap = $tahaps->first();
    $lastTahap = $tahaps->last();
    $defaultNama = old('nama_periode', $firstTahap->nama_periode);
    $defaultMulai = old('tanggal_mulai', $firstTahap->tanggal_mulai->format('Y-m-d'));
    $defaultSelesai = old('tanggal_selesai', $lastTahap->tanggal_selesai->format('Y-m-d'));
    $defaultJumlah = old('jumlah_tahap', $tahaps->count());
    $defaultDeskripsi = old('deskripsi_tahap', $tahaps->pluck('deskripsi')->values()->all());
@endphp
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-4 sm:p-6 text-gray-900">
                ...
                <div class="mb-4 bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-800">
                    <p class="font-medium mb-1">Perhatian saat mengubah jumlah tahap:</p>
                    <ul class="list-disc list-inside space-y-1">
                        <li>Menambah tahap: tahap baru dibuat; tanggal seluruh tahap dibagi ulang rata.</li>
                        <li>Mengurangi tahap: tahap kelebihan beserta modul yang sudah diupload pada tahap tersebut akan dihapus.</li>
                        <li>Untuk mengatur tanggal per tahap secara detail, gunakan tombol Edit di baris tahap setelah menyimpan.</li>
                    </ul>
                </div>

                <form method="POST" action="{{ route('admin.tahap-penyusunan.periode.update') }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number"
                                       ...
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                       required>
                                <p class="mt-1 text-xs text-gray-500">Minimal 1, maksimal 10 tahap. Saat ini: {{ $tahaps->count() }} tahap.</p>
                                @error('jumlah_tahap')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="bg-blue-50 p-4 rounded-lg mb-4">
                            <p class="text-sm text-blue-800">
                                Sesuaikan deskripsi tiap tahap (apa yang harus dikumpulkan penyusun). Tanggal tiap tahap akan dibagi rata dari rentang tanggal periode.
                            </p>
                        </div>

                        <div id="deskripsi-tahap-container" class="space-y-4"></div>
...
(function () {
    ...
    const oldValues = @json($defaultDeskripsi);
